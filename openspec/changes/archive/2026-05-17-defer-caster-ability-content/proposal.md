...
- **Remove the two grant files** `catalog/units/human/apprentice/paths/{cleric,arch_mage}/abilities/silver.json` (and the now-empty `abilities/` directories). No ability is auto-granted on promotion anymore.
...
- **Catalog (JSON)**: delete `catalog/units/human/apprentice/paths/cleric/abilities/silver.json` and `.../arch_mage/abilities/silver.json` (+ empty `abilities/` dirs). Edit `catalog/abilities/greater_heal/greater_heal.json` and `catalog/abilities/arcane_bolt/arcane_bolt.json` (additive `description` only — still valid, still load).
