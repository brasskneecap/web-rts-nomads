...
### Requirement: Unit target A* failures use strike-count escalation

**Reason**: Replaced by drift mode (see `unit-movement` capability). The strike system converged correctly but burned A* budget during the 9-tick cycle. Drift replaces every per-tick A* with a single walkability check while preserving "always moving" UX.

**Migration**: The fields `UnreachableTargetID` and `UnreachableStrikeCount` are removed from `Unit`. The function `applyUnitUnreachableEscalationLocked` is deleted. Scoring checks at `combat_ai_scoring.go:94` and `combat_ai_scoring.go:146` (skip-by-memo for unit candidates) are deleted. Tests in `combat_ai_unreachable_test.go` are deleted (they validated the removed system). `UnreachableUntilTick` is retained on `Unit` because the building memo system still uses it.
