...
Phase 1 landed the `caster` combat profile and the **inert** `AbilityCategory` data seam, but the autocast loop is still "first ready ability in slot order wins" and the Apprentice's only ability is `heal`. A caster cannot yet make a situational choice (heal a low ally vs. nuke an enemy), and the Cleric / Arch Mage promotion paths grant no abilities. Phase 2 activates the category seam (situational priority selection) and gives the promotion paths real kits, delivering the actual caster fantasy from `docs/design/caster_archetype.md`.
...
- **No-regression invariant (load-bearing):** with a single autocast-enabled ability (today's Apprentice = heal-only), highest-scored-ready collapses to exactly the current behaviour — same cast ticks for the same seed/inputs.
...
- New `path_ability_defs.go`: a loader twin of `path_defs.go` reading `catalog/units/human/apprentice/paths/<path>/abilities/<rank>.json` (shape `{ "grant": ["greater_heal"] }`) into `pathAbilityGrantsByKey[(path,rank)] []string`.
...
- The `Category` field becomes **active** — read by the priority scorer. This supersedes its Phase-1 "inert" guarantee (see Modified Capabilities). Not **BREAKING**: the no-regression invariant means single-ability units (incl. an un-promoted Apprentice) behave exactly as before.
...
- **Catalog (JSON)**: new `catalog/units/human/apprentice/paths/{cleric,arch_mage}/abilities/{bronze,silver,gold}.json`; new ability defs for the authored kit abilities (e.g. `catalog/abilities/greater_heal/greater_heal.json`).
...
- **Not breaking**: the no-regression invariant guarantees an un-promoted, heal-only Apprentice is byte-identical pre/post for the same seed; only multi-ability (promoted) casters exercise the new selection.
