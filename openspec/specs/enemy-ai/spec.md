...
Defines the enemy/combat AI re-evaluation cadence controls: the guard grace
window scaling, the global rate limit on map-wide objective searches, the
per-unit re-evaluation cooldown after failed acquisition, and the deterministic
target-drop retarget stagger that spreads mass retargets across ticks.
