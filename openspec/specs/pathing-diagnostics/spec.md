# pathing-diagnostics Specification

## Purpose

Defines the lightweight, allocation-free pathing diagnostics surface carried on
each unit (`RepathCount`, `StuckTriggerCount`, `LastStuckTick`), how those
counters reach the match snapshot, how persistently-stuck units are derived at
snapshot time, and the profiling instrumentation wrapped around player command
handlers.
