...
func TestPerksRoute_PostValid_RoundTrips(t *testing.T) {
	mux := newPathPerkTestMux(t)
	saveSyntheticUnit(t, mux, "route_test_unit_b", nil)

	pathBody := `{"unit":"route_test_unit_b","path":{"path":"route_test_path_b","ranks":{"bronze":{"maxHPMultiplier":1.1,"damageMultiplier":1.1,"attackSpeedMultiplier":1.0,"moveSpeedMultiplier":1.0,"attackRangeMultiplier":1.0}}}}`
	rec := doJSON(t, mux, http.MethodPost, "/paths", pathBody)
	if rec.Code != http.StatusCreated {
		t.Fatalf("setup POST /paths status=%d body=%s", rec.Code, rec.Body.String())
	}
	t.Cleanup(func() { doJSON(t, mux, http.MethodDelete, "/paths/route_test_path_b", "") })

	perkBody := `{"unit":"route_test_unit_b","path":"route_test_path_b","rank":"bronze","perks":[{"id":"route_test_perk_b","displayName":"Route Test Perk"}]}`
	rec = doJSON(t, mux, http.MethodPost, "/perks", perkBody)
	if rec.Code != http.StatusCreated {
		t.Fatalf("POST /perks status=%d body=%s", rec.Code, rec.Body.String())
	}
	t.Cleanup(func() { doJSON(t, mux, http.MethodDelete, "/perks/route_test_unit_b/route_test_path_b/bronze", "") })
	...
	if saveResp.Unit != "route_test_unit_b" || saveResp.Path != "route_test_path_b" || saveResp.Rank != "bronze" || saveResp.Status != "saved" {
		t.Errorf("save response = %+v, want unit/path/rank echoed + status=saved", saveResp)
	}
}

// saveSyntheticPath POSTs a minimal valid path file (one bronze rank row)
// for unitType, so perk-pool tests can satisfy the "path must exist first"
// ordering invariant (see Fix 3 / perk_editor.go's SaveEditorPerkPool).
func saveSyntheticPath(t *testing.T, mux *http.ServeMux, unitType, pathID string) {
	t.Helper()
	body := `{"unit":"` + unitType + `","path":{"path":"` + pathID + `","ranks":{"bronze":{"maxHPMultiplier":1.1,"damageMultiplier":1.1,"attackSpeedMultiplier":1.0,"moveSpeedMultiplier":1.0,"attackRangeMultiplier":1.0}}}}`
	rec := doJSON(t, mux, http.MethodPost, "/paths", body)
	if rec.Code != http.StatusCreated {
		t.Fatalf("setup POST /paths(%s/%s) status=%d body=%s", unitType, pathID, rec.Code, rec.Body.String())
	}
}

func TestPerksRoute_DuplicateIDAcrossLocations_Returns400(t *testing.T) {
	mux := newPathPerkTestMux(t)
	saveSyntheticUnit(t, mux, "route_test_unit_c", nil)
	saveSyntheticPath(t, mux, "route_test_unit_c", "path_one")
	saveSyntheticPath(t, mux, "route_test_unit_c", "path_two")
	t.Cleanup(func() {
		doJSON(t, mux, http.MethodDelete, "/perks/route_test_unit_c/path_one/bronze", "")
		doJSON(t, mux, http.MethodDelete, "/paths/path_one", "")
		doJSON(t, mux, http.MethodDelete, "/paths/path_two", "")
	})

	first := `{"unit":"route_test_unit_c","path":"path_one","rank":"bronze","perks":[{"id":"route_test_dup_perk"}]}`
	rec := doJSON(t, mux, http.MethodPost, "/perks", first)
	if rec.Code != http.StatusCreated {
		t.Fatalf("setup POST /perks(first) status=%d body=%s", rec.Code, rec.Body.String())
	}

	second := `{"unit":"route_test_unit_c","path":"path_two","rank":"bronze","perks":[{"id":"route_test_dup_perk"}]}`
	rec = doJSON(t, mux, http.MethodPost, "/perks", second)
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("POST /perks(dup id, different location) status=%d, want 400; body=%s", rec.Code, rec.Body.String())
	}
	if !strings.Contains(rec.Body.String(), "validation_failed") {
		t.Fatalf("body missing validation_failed: %s", rec.Body.String())
	}
	if !strings.Contains(rec.Body.String(), "route_test_dup_perk") {
		t.Fatalf("body missing the duplicated perk id: %s", rec.Body.String())
	}
}

func TestPerksRoute_DeleteSavedPool_ThenMalformedPathReturns400(t *testing.T) {
	mux := newPathPerkTestMux(t)
	saveSyntheticUnit(t, mux, "route_test_unit_d", nil)
	saveSyntheticPath(t, mux, "route_test_unit_d", "route_test_path_d")
	t.Cleanup(func() { doJSON(t, mux, http.MethodDelete, "/paths/route_test_path_d", "") })

	saveBody := `{"unit":"route_test_unit_d","path":"route_test_path_d","rank":"bronze","perks":[{"id":"route_test_perk_d"}]}`
	rec := doJSON(t, mux, http.MethodPost, "/perks", saveBody)
	if rec.Code != http.StatusCreated {
		t.Fatalf("setup POST /perks status=%d body=%s", rec.Code, rec.Body.String())
	}

	del := doJSON(t, mux, http.MethodDelete, "/perks/route_test_unit_d/route_test_path_d/bronze", "")
	if del.Code != http.StatusOK {
		t.Fatalf("DELETE /perks/.../bronze status=%d body=%s", del.Code, del.Body.String())
	}
	if !strings.Contains(del.Body.String(), `"deleted"`) {
		t.Fatalf("expected deleted status, got: %s", del.Body.String())
	}

	malformed := doJSON(t, mux, http.MethodDelete, "/perks/foo", "")
	if malformed.Code != http.StatusBadRequest {
		t.Fatalf("DELETE /perks/foo status=%d, want 400; body=%s", malformed.Code, malformed.Body.String())
	}
	if !strings.Contains(malformed.Body.String(), "invalid_id") {
		t.Fatalf("malformed delete body missing invalid_id: %s", malformed.Body.String())
	}
}
