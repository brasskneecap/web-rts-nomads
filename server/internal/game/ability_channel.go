...
func (s *GameState) tickUnitChannelLocked(unit *Unit, dt float64) {
	...
	for unit.ChannelNextTickIn <= 0 {
		...
		s.applyUnitDamageWithSourceLocked(target, def.DamagePerTick, DamageSource{
			AttackerUnitID: unit.ID,
			Kind:           "ability",
			DamageType:     def.DamageType.OrPhysical(),
		})

		// Compute heal amount and distribute via siphon heal logic.
		healAmount := int(math.Round(float64(def.DamagePerTick) * def.HealingMultiplier))
		if healAmount > 0 {
