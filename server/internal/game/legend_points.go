...
func (s *GameState) rollLegendPointDropLocked(attackerOwnerID string, deadUnit *Unit) {
	...
	if s.rngLoot.Float64() < chance {
		if tuning.LegendPoints.CommitMode == legendPointCommitModeImmediate {
			// Fire-and-forget commit; do NOT accumulate on the player so the
			// match-end commit path sees zero and cannot double-credit.
			if s.onLegendPointDropImmediate != nil {
				s.onLegendPointDropImmediate(attackerOwnerID, amount)
			} else {
				log.Printf("[LP] WARNING: commitMode=immediate but no hook wired; drop is lost (attacker=%s amount=%d)",
...
// SetImmediateLegendPointDropHandler wires the fire-and-forget commit hook
// invoked when gameplay tuning's legendPoints.commitMode is "immediate".
// Passing nil disables the immediate path (the default — tests do not need
// it).
func (s *GameState) SetImmediateLegendPointDropHandler(fn func(playerID string, amount int)) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.onLegendPointDropImmediate = fn
}
