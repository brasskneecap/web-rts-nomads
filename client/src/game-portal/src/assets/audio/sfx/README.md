...
`assets/buildings/<building>/sounds.json`, which maps an event name to a
...
`barracks_select.mp3` here, and plays it. A building with no `sounds.json`, or
no entry for the event, simply makes no sound.
