# Siphoner (Acolyte promotion path) — art placeholder

This directory is reserved for the Siphoner promotion variant's sprites. It is
intentionally empty of art for now.

Until a PixelLab export is dropped in here, Siphoner acolytes render with the
...
1. Drop the PixelLab export into this folder so it looks like the existing
   variants (see `../cleric/` for the shape):
   - `metadata.json`
...
The keyed sprite id is the directory name (`siphoner`), which must match the
`"path"` field in the server catalog.
