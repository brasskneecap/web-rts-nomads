# Arch Mage (Adept promotion path) — art placeholder

This directory is reserved for the Arch Mage promotion variant's sprites. It is
intentionally empty of art for now.

Until a PixelLab export is dropped in here, Arch Mage adepts render with
the base `adept` sprite (the runtime loader in `unitSprites.ts` falls back
...
3. If the new sprites are a different pixel size than the base Adept,
